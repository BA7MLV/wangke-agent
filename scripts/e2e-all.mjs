...
const META = {
  ...
  // 课程助手的课程上下文纯逻辑：courseId 规范化（去空白 / 去重 / 上限 5 门）、
  // 非法参数不得被当成空数组误清上下文、检索范围优先级（显式 courseId > 当前上下文 > 全库）。
  // 守的是「模型把 courseIds 传成 undefined 或字符串时，静默清空用户已选课程」这类错账 ——
  // 它在界面上看起来只是助手又开始满库乱搜，没有任何报错。
  'test-course-context': { service: 'none', antd: false, timeout: 120 },
