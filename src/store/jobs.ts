 ...
 * 都读同一份 job，切到哪儿进度都还在。
 *
 * v9 起这里也承载**阅读材料的解析**（`parse` phase）：
 * 三百页 PDF 的解析要几十秒，用户在库页点了导入就走开是常态，
 * 复用同一个「任务不跟组件走」的机制最省事，也自然获得列表行的进度展示。
 *
 * 2026-09-24：原来还有 `index` phase（材料文本块向量化）。稠密检索移除后它没有存在理由，
 * 已一并删掉 —— 材料的任务现在只有解析一段。
 */
